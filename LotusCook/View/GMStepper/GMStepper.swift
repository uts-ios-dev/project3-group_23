...
@IBDesignable public class GMStepper: UIControl {
    ...
    @IBInspectable public var stepCornerRadius: CGFloat = 4.0 {
        didSet {
            layer.cornerRadius = stepCornerRadius
            clipsToBounds = true
        }
    }
    
    /// Border width of the stepper and middle label's layer. Defaults to 0.0.
    @IBInspectable public var stepBorderWidth: CGFloat = 0.0 {
        didSet {
            layer.borderWidth = stepBorderWidth
            label.layer.borderWidth = stepBorderWidth
        }
    }
    
    /// Color of the border of the stepper and middle label's layer. Defaults to clear color.
    @IBInspectable public var stepBorderColor: UIColor = UIColor.clear {
        didSet {
            layer.borderColor = stepBorderColor.cgColor
            label.layer.borderColor = stepBorderColor.cgColor
        }
    }
